...
(async () => {
  try {
    ...
    // 2. Find and refresh VanHack tab
    const targetPage = pages.find(p => p.url().includes("vanhack.com"));
    if (!targetPage) {
      console.error("No active VanHack tab found!");
      return;
    }

    console.log(`Refreshing VanHack tab: ${targetPage.url()}`);

...
(async () => {
  try {
    ...
    // 2. Find and refresh Greenhouse tab
    const targetPage = pages.find(p => p.url().includes("greenhouse.io"));
    if (!targetPage) {
      console.error("No active Greenhouse tab found!");
      return;
    }

    console.log(`Refreshing Greenhouse tab: ${targetPage.url()}`);

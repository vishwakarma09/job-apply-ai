...
(async () => {
  try {
    ...
    // 2. Find and refresh ZipRecruiter tab
    const targetPage = pages.find(p => p.url().includes("ziprecruiter.com"));
    if (!targetPage) {
      console.error("No active ZipRecruiter tab found!");
      return;
    }

    console.log(`Refreshing ZipRecruiter tab: ${targetPage.url()}`);

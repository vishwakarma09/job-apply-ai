...
(async () => {
  try {
    ...
    // 2. Find and refresh CareerBeacon tab
    const targetPage = pages.find(p => p.url().includes("careerbeacon.com"));
    if (!targetPage) {
      console.error("No active CareerBeacon tab found!");
      return;
    }

    console.log(`Refreshing CareerBeacon tab: ${targetPage.url()}`);
    await targetPage.reload({ waitUntil: 'domcontentloaded' });
    await targetPage.waitForTimeout(4000);

    // 3. Open extension drawer if not already open
    console.log("Checking if extension drawer is open...");
    const drawer = targetPage.locator('#ai-job-apply-extension-root >> .drawer');
    const isOpen = await drawer.evaluate(el => el.classList.contains('open')).catch(() => false);
    if (!isOpen) {
      console.log("Drawer is closed. Opening extension drawer...");

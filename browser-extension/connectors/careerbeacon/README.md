# CareerBeacon Platform Connector Simulation & Automation

This directory contains the CareerBeacon integration connector for the AI Job Apply Assistant extension. It also contains monitoring and automation scripts to test and run the integration.
...
*   `monitor.js`: Playwright-based test script that opens a headed browser session with the extension loaded, navigates to CareerBeacon, and monitors the application forms, capturing snapshots and JSON state maps.
...
The monitor script launches a Google Chrome browser with the extension loaded, navigates to CareerBeacon, and prompts you to log in. Once logged in and on the job search page, press Enter in your terminal to start the automated loop:
```bash
node browser-extension/connectors/careerbeacon/monitor.js
```
...
```bash
node browser-extension/connectors/careerbeacon/turbo_apply.js
```

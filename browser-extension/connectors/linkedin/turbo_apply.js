...
(async () => {
  try {
    ...
    // 2. Find and refresh LinkedIn tab
    const targetPage = pages.find(p => p.url().includes("linkedin.com"));
    if (!targetPage) {
      console.error("No active LinkedIn tab found!");
      return;
    }

    console.log(`Refreshing LinkedIn tab: ${targetPage.url()}`);

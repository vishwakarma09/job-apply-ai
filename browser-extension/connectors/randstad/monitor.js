...
console.log("=== Randstad Job Application Monitor (Real Chrome Edition) ===");
...
setTimeout(async () => {
  try {
    ...
    console.log("Navigating to www.randstad.ca...");
    await page.goto('https://www.randstad.ca/', { waitUntil: 'domcontentloaded' }).catch(e => {
      console.log("Navigation warning:", e.message);
    });

    console.log("\n==================================================================");
    console.log("INSTRUCTIONS FOR USER:");
    console.log("1. Please log in to your Randstad account in the newly opened Chrome window.");
    ...
    console.log("3. Navigate to a Randstad job search page (e.g. searching for Software Developer).");
    console.log("4. Once logged in and on the search page, press ENTER in this terminal to continue.");
    console.log("==================================================================\n");

    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    await new Promise(resolve => rl.once('line', resolve));
    console.log("Resuming simulation... Initializing Randstad automation sequence.");
    ...
    async function scanPageForForms() {
      ...
      const targetPage = activePages.find(p => p.url().includes("randstad.ca") && p.url().match(/_([a-zA-Z0-9]+)\/?$/)) ||
                         activePages.find(p => p.url().includes("randstad")) || 
...
async function autoLaunchSingleApply(page, context) {
  ...
  while (true) {
    ...
    const targetPage = activePages.find(p => p.url().includes("randstad.ca") && p.url().match(/_([a-zA-Z0-9]+)\/?$/)) ||
                       activePages.find(p => p.url().includes("randstad.ca"));
    if (!targetPage) {
      continue;
    }

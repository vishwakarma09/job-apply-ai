...
(async () => {
  try {
    ...
    // 2. Find and refresh Randstad tab
    const targetPage = pages.find(p => p.url().includes("randstad.ca"));
    if (!targetPage) {
      console.error("No active Randstad tab found!");
      return;
    }

    console.log(`Refreshing Randstad tab: ${targetPage.url()}`);

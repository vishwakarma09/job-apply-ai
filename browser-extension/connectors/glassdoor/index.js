  ...
  EasyApply: {
    async automate(profile, logMessage, checkRunning, jobId = null) {
      ...
      const getActiveFormElement = () => {
        const selector = "div[role='dialog'], [class*='modal'], [id*='modal'], form[class*='apply'], form[id*='apply'], .glassdoor-apply-container";
      ...
      while (checkRunning()) {
        ...
        const successHeading = Array.from(activeForm.querySelectorAll('h1, h2, h3, h4, p, span')).find(el => {
          ...
          try {
            ...
            if (solverToken) {
              ...
              if (solveResponse && solveResponse.action === "fill" && solveResponse.fields) {
                logMessage("AI Solver successfully resolved Glassdoor fields.");
            ...
            console.warn("[AI Job Apply] AI Solver failed for Glassdoor:", solverErr);
          }
        }

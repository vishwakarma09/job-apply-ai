...
console.log("=== Glassdoor Job Application Monitor (Real Chrome Edition) ===");
...
setTimeout(async () => {
  try {
    ...
    console.log("Navigating to www.glassdoor.ca...");
    await page.goto('https://www.glassdoor.ca/', { waitUntil: 'domcontentloaded' }).catch(e => {
      console.log("Navigation warning:", e.message);
    });

    console.log("\n==================================================================");
    console.log("INSTRUCTIONS FOR USER:");
    console.log("1. Please log in to your Glassdoor account in the newly opened Chrome window.");
    ...
    console.log("3. Navigate to a Glassdoor job search page (e.g. searching for Software Engineer).");
    console.log("4. Once logged in and on the search page, press ENTER in this terminal to continue.");
    console.log("==================================================================\n");

    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
    await new Promise(resolve => rl.once('line', resolve));
    console.log("Resuming simulation... Initializing Glassdoor automation sequence.");
    ...
    async function scanPageForForms() {
      ...
      const targetPage = activePages.find(p => p.url().includes("glassdoor")) || activePages[activePages.length - 1] || page;
...
async function autoLaunchSingleApply(page, context) {
  ...
  while (true) {
    ...
    const targetPage = activePages.find(p => p.url().includes("glassdoor.ca") || p.url().includes("glassdoor.com"));
    if (!targetPage) {
      continue;
    }
    ...
    try {
      ...
      // We won't automatically click start to avoid interfering with manual inspection unless requested
      console.log("[AutoLaunch] Drawer opened successfully. You can now use Single Apply or Turbo Mode manually.");
      break;
    } catch (e) {

...
(async () => {
  try {
    ...
    // 2. Find and refresh Glassdoor tab
    const targetPage = pages.find(p => p.url().includes("glassdoor.ca") || p.url().includes("glassdoor.com"));
    if (!targetPage) {
      console.error("No active Glassdoor tab found!");
      return;
    }

    console.log(`Refreshing Glassdoor tab: ${targetPage.url()}`);
